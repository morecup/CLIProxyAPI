...
// fallbackRoundTripper selects the transport required by each protected HTTPS
// host and falls back to the standard transport for all other requests.
...
// NewUtlsHTTPClient creates an HTTP client using a Chrome profile only for
// ChatGPT. Anthropic uses the standard transport until a separately verified
// Claude Desktop transport profile is available.
...
// NewUtlsHTTPClientWithProxyURL creates a client pinned to one already-resolved
// egress setting. It does not consult mutable auth or global configuration.
func NewUtlsHTTPClientWithProxyURL(ctx context.Context, proxyURL string, timeout time.Duration) *http.Client {
	...
	// Anthropic stays on the standard transport until a captured Desktop
	// transport profile is available.
	anthropicRT := standardTransport
