...
func TestDeleteClaudeKey_RequiresBaseURLWhenAPIKeyDuplicated(t *testing.T) {
	t.Parallel()
	...
	c.Request = httptest.NewRequest(http.MethodDelete, "/v0/management/claude-api-key?api-key=shared-key", nil)
	...
	if got := len(h.cfg.ClaudeKey); got != 2 {
		t.Fatalf("claude keys len = %d, want 2", got)
	}
}

func TestDeleteClaudeKey_DeletesOnlyMatchingBaseURL(t *testing.T) {
	t.Parallel()
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodDelete, "/v0/management/claude-api-key?api-key=shared-key&base-url=https://a.example.com", nil)
	...
	if got := h.cfg.ClaudeKey[0].BaseURL; got != "https://b.example.com" {
