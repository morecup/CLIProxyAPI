...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	claudeAPIKeyCount := BuildAPIKeyClients(cfg)
	totalAPIKeyClients := claudeAPIKeyCount
	...
	totalNewClients := authFileCount + claudeAPIKeyCount
	...
	log.Infof("full client load complete - %d clients (%d auth files + %d Claude API keys)",
		totalNewClients,
		authFileCount,
		claudeAPIKeyCount,
	)
