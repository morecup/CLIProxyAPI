...
var zhStrings = map[string]string{
	...
	"oauth_magic_link":             "邮件魔法链接",
	"oauth_magic_link_placeholder": "https://claude.ai/magic-link?client=desktop#...",
	"oauth_magic_link_hint":        "  从邮件复制完整 Claude Desktop 魔法链接并粘贴到下方；不要先在外部浏览器中打开。代理会在临时隔离的 WebView 中加载官方页面完成验证。链接仅用于本次认证，不会写入磁盘。",
	"oauth_magic_link_press_c":     "  按 [c] 重新粘贴魔法链接 • [Esc] 返回",
...
var enStrings = map[string]string{
	...
	"oauth_callback_placeholder":   "http://localhost:.../auth/callback?code=...&state=...",
	"oauth_magic_link":             "Email magic link",
	"oauth_magic_link_placeholder": "https://claude.ai/magic-link?client=desktop#...",
	"oauth_magic_link_hint":        "  Copy the complete Claude Desktop magic link from your email and paste it below; do not open it first in an external browser. The proxy loads the official page in a temporary isolated WebView to complete verification. The link is used only for this authentication and is never written to disk.",
	"oauth_magic_link_press_c":     "  Press [c] to paste the magic link again • [Esc] to go back",
