...
On non-Windows hosts, Claude Desktop magic-link attestation launches a local Chromium or Chrome process with an isolated temporary profile. Install `chromium`, `chromium-browser`, `google-chrome-stable`, or `google-chrome`, or set `CLIPROXY_CLAUDE_DESKTOP_CHROMIUM_PATH` to the executable. Headless mode is enabled by default; set `CLIPROXY_CLAUDE_DESKTOP_CHROMIUM_HEADLESS=false` only when an interactive display is available. The browser uses the configured `proxy-url` through a loopback-only CONNECT bridge so proxy credentials are not placed in the browser command line.
